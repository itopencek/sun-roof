...
class BranchServiceTest {
    ...
    void findByEmployeeId() {
        ...
        BDDMockito.given(branchRepository.findBranchByEmployees(Optional.of(employee5))).willReturn(branches);

        Assertions.assertEquals(branchesDTO, branchService.findByEmployeeId(employee5.getId()));

        Mockito.verify(branchRepository, Mockito.atLeastOnce()).findBranchByEmployees(Optional.of(employee5));
    }
